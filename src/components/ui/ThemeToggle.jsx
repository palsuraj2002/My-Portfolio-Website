...
const ThemeToggle = () => {
  ...
  return (
    <label className="btn btn-ghost btn-circle swap swap-rotate">
      ...
      {/* Sun Icon (shows when theme is dark, because we want to click to go light) */}
      <svg
        className={`swap-on fill-current w-5 h-5 ${theme === "dark" ? "block" : "hidden"}`}
      ...
      <svg
        className={`swap-off fill-current w-5 h-5 ${theme === "light" ? "block" : "hidden"}`}
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
      >
